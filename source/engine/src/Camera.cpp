...
{
    const glm::vec3 camRight = glm::normalize(glm::vec3(transformation[0][0], transformation[0][1], transformation[0][2]));
    const glm::vec3 camUp = glm::normalize(glm::vec3(transformation[1][0], transformation[1][1], transformation[1][2]));
    const glm::vec3 camFront = glm::normalize(glm::vec3(transformation[2][0], transformation[2][1], transformation[2][2]));

    const glm::vec3 globalPosition = glm::vec3(transformation[3]);
    m_viewMat = glm::lookAt(globalPosition, glm::normalize(globalPosition + camFront), glm::vec3(0.0f, 1.0f, 0.0f));
...
void Loops::Camera::UpdateCamera(const Loops::Transform& transform)
{
    ...
    const glm::vec3 camRight = glm::normalize(glm::vec3(matrix[0]));// [0] , matrix[0][1], matrix[0][2]));
    const glm::vec3 camUp = glm::normalize(glm::vec3(matrix[1]));// [0] , matrix[1][1], matrix[1][2]));
    const glm::vec3 camFront = glm::normalize(glm::vec3(matrix[2]));// [0] , matrix[2][1], matrix[2][2]));

    const glm::vec3 globalPosition = glm::vec3(matrix[3]);
    m_viewMat = glm::lookAt(globalPosition, glm::normalize(globalPosition + camFront), glm::vec3(0.0f, 1.0f, 0.0f));
}
