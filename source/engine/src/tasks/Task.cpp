...
void Common::GraphicsTask::CreateAttachments()
{
    assert(0);
}

Common::GraphicsTask::GraphicsTask(const char* name, const GraphicsTaskInfo& info) : m_info(info), m_ownAttachments(true)
{
    strncpy(m_name, name, sizeof(m_name) - 1);
    CreateAttachments();
}

Common::GraphicsTask::GraphicsTask(const char* name, const GraphicsTaskInfo& info, const std::vector<VkImageView>& colorViews,
...
Common::GraphicsTask::~GraphicsTask()
{
    if (m_ownAttachments)
    {
        assert(0);
...
const std::vector<VkImageView>& Common::GraphicsTask::GetColorAttachmentViews() const
{
    return m_colorAttachmentViews;
}

const std::vector<VkImageView>& Common::GraphicsTask::GetDepthAttachmentViews() const
{
    return m_depthAttachmentViews;
}

void Common::GraphicsTask::Submit(uint32_t frameInFlight, const VkSemaphore& timelineSem, uint64_t signalValue,
    std::optional<uint64_t> waitValue)
{
    ...
    //if (m_alive)
    {
        ErrorCheck(vkQueueSubmit2(m_info.m_graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE));
    }
}